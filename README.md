...
## 1. 🧩 Рефакторинг UI: разбивка на виджеты

### Проблема
- Экран пейволла и главный экран — монолитные виджеты
- Сложно тестировать, переиспользовать, читать

### Решение
Разбить каждый экран на атомарные виджеты:

## 2. ❄️ Добавление Freezed для иммутабельных моделей
### Проблема
- Ручные `copyWith` и `toJson` — боль при добавлении полей
- Нет `toString`, `hashCode`, `==` автоматически

### Решение
Добавить `freezed` + `json_serializable`:

## 3. 👤 Регистрация / Авторизация пользователя
Почему нужно
Привязать подписку к конкретному пользователю

Отслеживать историю платежей

## 4. 🔄 Реальная подписка с автоматическим продлением
Проблема
Сейчас эмуляция, нет реального бэкенда

Нет проверки на истечение подписки
